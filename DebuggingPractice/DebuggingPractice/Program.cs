...
namespace DebuggingPractice
{
    class Program
    {
        ...
        {
            Console.WriteLine(IsPalindrome("firetruck"));
            Console.WriteLine(IsPalindrome("radar"));
            Console.WriteLine(IsPanagram("the quick brown fox jumps over the lazy dog"));
            Console.WriteLine(IsPanagram("This house is brown"));
            Console.WriteLine(IsValidISBN("0-7475-3269-9"));
        ...
        public static bool IsPalindrome(string input)
        {
            ...
            for (int i = 0; i < input.Length; i++)
            {
                // check the character to see if its not equal
                // to its partner
                if (input[i].ToString().ToLower() != input[input.Length - i - 1].ToString().ToLower())
                {
                    //not a palindrome
                    return false;
                }
